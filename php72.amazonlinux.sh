...
yum install -y git make gcc gcc-c++ libxml2-devel pkgconfig openssl-devel \
...
docker run --rm --name amazonlinux-$VERSION -i -v `pwd`/pkg:/pkg amazonlinux:1 /pkg/build.sh
